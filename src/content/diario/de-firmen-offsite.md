---
locale: de
grupo: corporate-venue
slug: firmen-offsite-am-rio-arapiuns
titulo: "Firmen-Offsite am Rio Arapiuns"
tituloCurto: "Firmen-Offsite"
descricao: "Exklusivnutzung der gesamten Anlage im Amazonasgebiet für Firmenteams: dreizehn Bangalôs, überdachter Pavillon für das Plenum, Speisesaal und Privatstrand. Angebot pro Gruppe."
resposta: "Villa Arapiuns empfängt Firmenteams zur Exklusivnutzung der gesamten Anlage: dreizehn Bangalôs für bis zu sechsundzwanzig Personen, ein überdachter Palmpavillon für das Plenum, ein Speisesaal, in dem das ganze Team zusammen isst, und ein Privatstrand am Rio Arapiuns, anderthalb Bootsstunden von Alter do Chão. Vollpension, Solarenergie und ein lokales Team. Die Preise werden pro Gruppe kalkuliert, nach Personenzahl und Nächten."
data: 2026-08-29
imagem: pavilhao-palha
imagemAlt: "Ein an allen Seiten offener Holzpavillon mit Palmdach und Holzterrasse, umgeben von Wald, mit Solarpaneelen auf dem Dachfirst"
legenda: "Der Pavillon · der Plenumsraum, an allen vier Seiten offen"
destino: privateVilla
duvidas:
  - q: "Kann das Team von dort online arbeiten?"
    a: "Ja. Vor Ort gibt es Starlink mit stabiler Solarversorgung, und es trägt das ganze Team gleichzeitig durch eine Online-Besprechung. Wenn Ihr Programm mitten in der Woche einen Call mit der Zentrale braucht, findet er statt — ohne dass jemand für Empfang auf eine Böschung steigt."
  - q: "Welche Präsentationstechnik gibt es?"
    a: "Vor Ort gibt es keine Ton- oder Videotechnik. Wenn Ihr Programm Projektion, Ton oder Aufzeichnung braucht, wird die Produktion separat beauftragt, und das sollte von Anfang an im Budget stehen. Der Pavillon bietet Dach, Schatten und die ganze Gruppe an einem Ort — alles Weitere bringen Sie mit."
  - q: "Stellen Sie Rechnungen aus und arbeiten Sie mit Vertrag?"
    a: "Ja. Wir stellen Rechnungen aus und arbeiten mit einem Gruppenvertrag. 30 % Anzahlung sichern den Termin, der Restbetrag ist spätestens sieben Tage vor Anreise vollständig fällig, per Karte oder PIX. Der Vertrag sieht ein 48-stündiges Widerrufsfenster mit voller Rückerstattung vor; danach werden bei Stornierung mehr als sechzig Tage vor Anreise 30 % des Gesamtbetrags einbehalten, und ab sechzig Tagen oder weniger erfolgt keine Rückerstattung."
  - q: "Wie viele Personen passen hinein?"
    a: "Dreizehn Bangalôs, bis zu sechsundzwanzig Personen bei Exklusivnutzung der Anlage — an diesen Terminen ist niemand außerhalb Ihres Teams vor Ort. Wir kalkulieren Gruppen unterschiedlicher Größe; sagen Sie uns, wie viele Sie sind, und Sie bekommen eine Zahl."
  - q: "Wie viele Nächte sind sinnvoll?"
    a: "Die Anreise dauert einen ganzen Tag, die Rückreise ebenso. Ab drei Nächten geht die Rechnung auf. Für eine einzige Nacht nicht — und wir sagen das lieber, bevor Sie das Budget aufstellen."
  - q: "Wann ist das beste Zeitfenster?"
    a: "Februar bis Juni ist Hochwasser: keine Strände, aber Stille, überfluteter Wald und die größte Verfügbarkeit für die ganze Anlage. Wochentage zwischen September und November bringen das beste Wetter des Jahres mit breiten Stränden. Dezember und Januar sind Hochsaison und zuerst ausgebucht."
---

Wenn Sie entscheiden, wohin Sie ein Team mitnehmen, ist die entscheidende Frage nicht, ob der Ort schön ist. Sie lautet, ob er Ihr Programm trägt, ob sich die Anreise rechnet und ob Sie ihn buchen können, ohne dass es für Ihre Finanzabteilung zum Problem wird.

Diese Seite beantwortet alle drei — auch in den Teilen, die uns von Ihrer Liste streichen könnten.

## Was der Ort ist

Dreizehn Bangalôs am Ufer des Rio Arapiuns im Bundesstaat Pará, innerhalb des Extraktivreservats Tapajós-Arapiuns — anderthalb Bootsstunden flussaufwärts von Alter do Chão. Bis zu sechsundzwanzig Personen. Aufgeständertes Holz, Palmdächer, Solarenergie und ein Team, das hier lebt.

Bei Exklusivnutzung ist an diesen Terminen niemand außerhalb Ihres Teams vor Ort. Das ist der Unterschied zwischen einem Offsite und einer Incentive-Reise, bei der man das Frühstück mit Fremden teilt.

## Wo das Programm stattfindet

**Der Pavillon** ist der Plenumsraum: eine lange, palmgedeckte Konstruktion, an allen vier Seiten offen, in der das gesamte Team zusammen sitzt. Hier haben die lange Session, die Präsentation und die Strategiediskussion Platz.

**Die Shala** ist der zweite Raum, geschlossen genug, damit sich eine kleinere Gruppe zurückziehen kann.

**Der Speisesaal** — alle an einem Tisch, dreimal am Tag, über mehrere Tage. Für ein Team ist das ebenso Arbeitsraum wie der Pavillon. Hier entstehen die Gespräche, die nicht in die Agenda passen.

**Der Strand und der Hängemattenhain** sind die Orte, an denen Menschen es schaffen, nicht in einer Session zu sein. Ein Offsite ohne diese beiden wird zu einer langen Sitzung an einem teuren Ort.

## Die Verbindung funktioniert — Abgeschiedenheit ist Ihre Entscheidung

Hier lohnt Deutlichkeit, denn genau daran scheitern die meisten Firmenentscheidungen.

**Es gibt Starlink mit stabiler Solarversorgung, und es trägt das ganze Team gleichzeitig durch eine Online-Besprechung.** Niemand muss für Empfang auf eine Böschung steigen, und der Call mit der Zentrale mitten in der Woche findet statt. Falls das Ihre Frage war: sie ist beantwortet.

Was sich ändert, ist die Frage, wer entscheidet. Abgeschiedenheit kommt hier nicht aus der Geografie, sondern von Ihnen. Wenn das Ziel ist, das Team für einige Tage aus dem Betrieb zu nehmen, wird das zu einer Regel, die die Führung setzt und ankündigt — nicht zu einer Einschränkung, die der Ort auferlegt. Unsere Empfehlung: klären Sie das vor der Anreise, denn ein Team ohne Absprache nutzt das Netz genauso wie im Büro.

## Der Inhalt, den der Ort bereits mitbringt

Der Teil des Programms, den wir nicht erfinden müssen: Kanufahrt durch den überfluteten Wald, der Samaúma-Pfad von der Gemeinde Atodi aus, die Herstellung von Maniokmehl, Palmflechten mit den Kunsthandwerkerinnen, nächtliches Tauchfischen, gelehrt von Fischern der Region.

Geleitet werden sie von den Familien, die davon leben, und direkt an sie bezahlt — das Team sollte also Bargeld in Reais mitbringen. Das ist keine von einer Beratung entworfene Teambuilding-Übung, sondern echte Arbeit, ausgeführt von denen, die sie tun, mit dem Team daneben. Bei den meisten Gruppen entsteht daraus mehr Gespräch als bei jeder konstruierten Übung.

Die **Piracaia** — im Sand gegarter Fisch im Bananenblatt, bei Kerzenlicht, im Kreis am Strand serviert — beschließt meist den letzten Abend, ohne dass jemand eine Abschlusszeremonie entwerfen müsste.

## Anreise

Flug nach Santarém (STM), direkt aus Manaus, Belém und Brasília, mit Anschluss ab São Paulo. Danach 40 Minuten mit dem Auto und 90 Minuten mit dem Boot.

Rechnen Sie an jedem Ende mit einem vollen Reisetag. Das Boot organisieren wir als Teil des Gruppenpakets; die Flüge liegen bei Ihnen.

## Was es nicht gibt

Kein Spa, kein Fitnessraum, kein geschlossener Besprechungsraum mit Konferenztechnik. Es gibt einen Pavillon, einen Speisesaal, einen Strand und einen Wald.

Keine Strände von Februar bis Juni — der Fluss steigt, der Sand verschwindet unter Wasser. Eine schöne, andere Jahreszeit, aber verkaufen Sie Ihrem Team im April keinen Strand.

Und wir sind kein Ort für Plant Medicine. Ausdrücklich gesagt, weil das Amazonasgebiet diese Annahme nahelegt.

## Wie wir kalkulieren

Der Übernachtungspreis richtet sich nach Gruppengröße und Anzahl der Nächte und sinkt, je größer die Gruppe ist. Wir kalkulieren pro Gruppe, statt eine feste Tabelle zu veröffentlichen.

Schicken Sie uns die Termine, die Personenzahl, die Anzahl der Nächte und das, was Ihr Programm wirklich von einem Ort braucht. Sie bekommen eine echte Zahl zurück.

## Vertragliches

Wir stellen Rechnungen aus und arbeiten mit einem Gruppenvertrag. **30 % Anzahlung** sichern den Termin, der **Restbetrag ist sieben Tage vor Anreise vollständig fällig**. Wir akzeptieren **Karte und PIX**.

**Achtundvierzig Stunden Widerrufsfrist**: Wer in diesem Zeitraum storniert, nach Unterschrift und Anzahlung, erhält den vollen Betrag zurück. Danach werden **bei Stornierung mehr als sechzig Tage vor Anreise 30 % des Gesamtbetrags einbehalten**, und **ab sechzig Tagen oder weniger gibt es keine Rückerstattung**. Es ist die letzte Frage der Finanzabteilung des Kunden und die, die am häufigsten gegen ein Landhotel entscheidet — deshalb steht sie hier und nicht nur im Vertrag.
